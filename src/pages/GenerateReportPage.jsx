  ...
  const handleGeneratePDF = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/reports?startDate=${startDate}&endDate=${endDate}`,
        {
          headers: {
      ...
      <form onSubmit={handleGeneratePDF} className="p-4 space-y-6">
        <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
          <h3 className="text-lg font-semibold text-slate-100 mb-4 flex items-center">
            <Calendar size={20} className="inline mr-2 text-red-400" /> Select
            Date Range
          </h3>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full flex items-center justify-center py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl shadow-lg transition duration-200 active:scale-98 disabled:opacity-50"
        >
