...
import { format } from 'date-fns'; // Required for date display in the new section

import { 
    Briefcase, UserPlus, RefreshCw, FileText, HelpCircle, Users, CreditCard, FileSearch 
} from 'lucide-react';
...
import CaseDiaryView from '../components/Diary/CaseDiaryView'; // <<< NEW Component Import

const Diary = () => {
    const [selectedDate, setSelectedDate] = useState(new Date());
    const navigate = useNavigate(); 

    // Features List Data (UPDATED LIST)
    const menuOptions = [
        {
            id: 1,
            title: "Add New Case", // <<< Separated
            desc: "Enter court details, case number, hearing dates.",
            icon: <Briefcase className="w-6 h-6 text-blue-400" />,
            path: "/add-case"
        },
        {
            id: 10, // <<< NEW BUTTON for Case Management
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white pb-24">
            ...
            {/* --- Section 2: Split Layout (New structure) --- */}
            <div className="p-4 grid grid-cols-1 lg:grid-cols-3 gap-6">

                {/* Left Side: Core Actions Menu */}
                <div className="lg:col-span-2"> {/* Takes 2/3 space on large screens */}
